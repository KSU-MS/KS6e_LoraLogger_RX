...
def comportPrompt():
    layout = [  [sg.Text("enter Teensy com port #")],     # Part 2 - The Layout
            ...
            [sg.Button('Ok')] ]
    # Create the window
    window = sg.Window('KS5e Parser', layout)      # Part 3 - Window Defintion
...
class serialPlot:
    ...
    def getSerialData(self, frame, lines, lineValueText, lineLabel, timeText,motorTemp):
...
class Window(Frame):
    ...
    def initWindow(self, figure):
        ...
        lbl1=Tk.Label(frame2,text="Quit")
        lbl1.pack(padx=5,pady=5)
        self.entry = Tk.Entry(frame2)
        self.entry.pack(padx=5)
        QuitButton=Tk.Button(frame2,text="quit",command=self.quit())
        QuitButton.pack(padx=5)
...
def main():
    ...
    anim = animation.FuncAnimation(fig, s.getSerialData, fargs=(lines, lineValueText, lineLabel, timeText, motorTemp), interval=pltInterval)    # fargs has to be a tuple
